...
+function($,getAge){
    ...
    function check(event) {
        ...
        if (rule||tipLength||customs) { 
            ...
            if($this.attr('data-blz-auto-check')!=='yes'){
                ...
                $this.data('blzTimer',setInterval(function(){
                    check.call($this);
                },800));
            }
            if(event){
	...
	$.fn.check = function(obj) {
		...
		var $elems = [this.eq(0).find('[data-blz-type]:not([disabled])')]; // 获取要验证的表单元素
		...
		$('#mf-btn-send').on('click.sendMessage',function(){
            if($(this).data('isCountStart')){return false;}
...
//                },500);
		...
		this.eq(0).on('submit.blz',function(event){
			...
			if($this.find('[data-blz-check="pass"]').length!==$this.find('[data-blz-type]:not([disabled])').length){
...
+function($){
   ...
   $(elemTab).on('click','.mt-nav-item a',function(e){
	    e.preventDefault();
   		$(this).closest('.mt-nav-item').addClass('active').siblings().removeClass('active');
		swiper.slideTo($(this).closest('.mt-nav-item').data('index'));
   });
}(window.jQuery);
...
+function($){
	...
	$(document).on('click','[data-cartoon]',function(){
		$($(this).attr('data-cartoon')).css('display','block').addClass('animation');
	});
	$(document).on('click','[data-cartoon-dismiss]',function(){
		$(this).closest($(this).attr('data-cartoon-dismiss')).css('display','none').removeClass('animation');
	});
}(window.jQuery);  